# PATCH Y · Example / YTI Pairs — v1.1

## Purpose
Patch weak Example/YTI transfer while preserving good paired members.
...
## Target selection
Only REBUILD Example/YTI items routed here may change. Read the full pair; edit only target member(s). KEEP partner remains read-only.

YTI must create meaningful transfer through unknown, representation, dependency, orientation, information order, or reasoning—not just nouns/numbers. When I-can naturally supports formula/math application, a strong contextual quantitative pair takes priority over generic conceptual/vocabulary filler. Avoid naked variable assignments. Supply actual representations when needed.

Changed item -> NEEDS AUDIT; clear old reasons; re-derive affected task metadata; mark metadata reconcile required; recompute summary.
