# PATCH Q · Core Questions — v1.1

## Purpose
Patch isolated substantive Physics/content/context/evidence failures not better owned by a destination-specific patch.
...
## Target selection
Only REBUILD items routed to Core Questions may change.

Use for physically incompatible context, missing/contradictory nonvisual evidence, incorrect Physics/key, fundamentally unsuitable question structure, or isolated semantic clone outside Practice. Rebuild from exact I-can + evidence job + current design map. If a more specific patch owns the defect, fail closed and name it.

Changed item -> NEEDS AUDIT; clear reasons; mark metadata reconcile required; recompute summary.
