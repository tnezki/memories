# PATCH E · Exit Tickets — v1.1

## Purpose
Patch red Exit items as fresh formative-secure evidence with real variety within each four-question ticket and between A/B/C.
...
## Target selection
Only REBUILD Exit Ticket items routed here may change. Treat Exit destination as formative-secure regardless of stale labels.

Plan all four questions on each affected ticket and A/B/C siblings together. Choose best-fit modes from the I-cans; do NOT force a fixed four-type quota. Each patched item must be semantically independent of exposed Notes/Practice/CYU/Warm-Up and sibling Exit forms. New object/number/name/order is not independence. Preserve exact slot/form/I-can/evidence routing and counts.
