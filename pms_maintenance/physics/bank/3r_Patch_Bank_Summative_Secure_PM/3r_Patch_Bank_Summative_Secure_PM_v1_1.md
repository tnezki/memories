# PATCH S · Summative Secure Items — v1.1

## Purpose
Patch red secure Summative items while preserving blueprint, mappings, response mode, form slots, and overall architecture.
...
## Target selection
Only REBUILD Summative items routed here may change.

Preserve form/version, slot/order, exact I-can/evidence routing, SR/CR mode, point intent, counts, and coverage. Build genuinely new evidence, compare against exposed instruction/Exits/other forms, and make V5/V6 true alternates. Fix response-mode mismatch: an SR stem must be answerable by selecting an option, not secretly require drawing/writing first. If representation-based, use secure evidence appropriate to the item. For SR items, supply coherent item-specific distractors; if the secure stem is already good and only distractors are defective, that belongs to PATCH D instead.

Changed items -> NEEDS AUDIT; clear reasons; mark metadata reconcile required; recompute summary.
