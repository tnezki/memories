# PATCH F · Cross-Resource Freshness — v1.1

## Purpose
Patch audited cross-resource reuse/freshness failures while preserving good local Bank questions and destination purpose.
...
## Target selection
Only REBUILD items routed to Cross-Resource Freshness may change.

Compare the current item against the relevant exposed/secure resources packaged or represented in the current Bank. Patch only the item(s) whose reuse violates the current Framework freshness/security contract. Do not chase harmless shared concepts or legitimate family repetition. A new noun/number/order is not freshness.

Changed item -> NEEDS AUDIT; clear reasons; mark metadata reconcile required when family/variation changes; recompute summary.
