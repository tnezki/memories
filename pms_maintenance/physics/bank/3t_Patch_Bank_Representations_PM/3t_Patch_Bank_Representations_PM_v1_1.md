# PATCH R · Representations — v1.1

## Purpose
Patch isolated red representation failures not already owned by WTC, Practice, Exit, Summative, or another destination-specific patch.
...
## Target selection
Only REBUILD items explicitly routed to Representation may change.

Decision order: good question + bad visual -> patch visual only; good visual + bad binding/wording -> patch binding/wording only; both defective -> patch both only as needed. Actual representation must carry requested evidence, be answer-neutral where required, and match prompt/key. Inline SVG and external figures receive the same QA. Remove/reject prohibited source visuals.

Changed item -> NEEDS AUDIT; clear reasons; mark metadata reconcile required when representation metadata/design linkage changes; recompute summary.
