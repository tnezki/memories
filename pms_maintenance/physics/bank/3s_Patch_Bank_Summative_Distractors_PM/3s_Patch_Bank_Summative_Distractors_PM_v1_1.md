# PATCH D · Summative Distractors — v1.1

## Purpose
Patch only selected-response Summative items whose secure stem/evidence is sound but distractors are generic, recycled, or implausible.
...
## Target selection
Only REBUILD SR Summative items routed here may change. If stem/stimulus/security is defective, fail closed and route to PATCH S.

Preserve sound stem/stimulus/correct answer. Build each distractor from a plausible exact-item misconception, sign/direction error, representation misread, relationship misuse, calculation error, or conceptual confusion. No generic distractor bags across unrelated families. Verify exactly one best answer.

Choice changes are material -> NEEDS AUDIT; clear reasons; recompute summary.
