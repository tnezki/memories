# PATCH M · Mapping Integrity — v1.1

## Purpose
Patch only audited mapping-integrity failures so later content patches can trust exact I-can/evidence routing.
...
## Target selection
Only CLEAN/REBUILD items explicitly routed to `patch_bank_mapping_integrity` may change. This PM is the only patch PM allowed to alter exact mapping fields.

## Allowed mapping repairs
- synchronize canonical JSON, I-can map, mapping/evidence CSVs, design map, and teacher views when they disagree mechanically;
- correct a semantically wrong primary/supporting I-can only when the exact intended target is unambiguous from Framework + actual question and the change preserves required destination/form coverage;
- repair stale evidence/security routing metadata when it is clearly inconsistent with the current destination contract.

## Fail closed
Do NOT invent a new learning target or remap merely to make coverage look balanced. If correcting the mapping would break a secure blueprint, required coverage, or reveal that the question itself is the real defect, do not force the mapping; leave item unresolved and route/report it to the correct content patch.

After a material mapping change, set item to NEEDS AUDIT, clear old reason fields, mark `metadata_reconcile_required=true`, and recompute routing summary.
