# PATCH P · Practice Families — v1.1

## Purpose
Patch only red Practice slots that over-clone finished tasks, misuse design quotas, or otherwise fail family variation—while freezing good Practice exemplars and counts.
...
## Target selection
Only REBUILD Practice items routed here may change. KEEP Practice is read-only and should be treated as useful exemplar evidence when appropriate.

Before writing replacements, map affected I-can -> existing good exemplars -> missing evidence opportunities -> representation/formula/tool opportunities -> replacement slot design. Use `_question_structure.zip`. Preserve 24-question destination counts and exact mapping/routing.

Do not force Standard/Inverted/Context-rich, response modes, or representations into fixed quotas. Choose best fit for each I-can. Vary evidence job/dependency/unknown/representation when meaningful, not merely names/numbers/units. Purposeful family repetition is allowed; excess finished-question clones are not.

Changed items -> NEEDS AUDIT; clear reasons; update design-map linkage as needed; mark metadata reconcile required; recompute summary.
