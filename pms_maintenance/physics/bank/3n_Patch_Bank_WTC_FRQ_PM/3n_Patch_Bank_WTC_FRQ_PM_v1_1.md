# PATCH W · WTC FRQs — v1.1

## Purpose
Patch red WTCs into the current connected FRQ-decomposition format without altering the locked Bank-card presentation.
...
## Target selection
Only REBUILD WTC items routed here may change.

Each WTC remains one coherent 3–5-part FRQ around a genuine shared situation/stimulus. Its primary purpose is practicing how to unpack/navigate a long Physics question, not proving current-section mastery. Cross-unit content is allowed under current Framework. Supply the actual graph/table/diagram/model/data if any part depends on it. Do not fake FRQ form with three unrelated mini-questions. Solutions must address the actual parts, not use generic boilerplate.

Keep card shell/metadata UI unchanged. Material WTC patch -> NEEDS AUDIT; clear old reasons; recompute summary.
