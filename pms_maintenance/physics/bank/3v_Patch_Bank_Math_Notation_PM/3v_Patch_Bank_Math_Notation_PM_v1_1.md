# PATCH N · Math / Notation — v1.1

## Purpose
Patch audited math, unit, symbol, MathJax, and notation defects without broad question reauthoring.
...
## Target selection
Only CLEAN/REBUILD items routed to Math / Notation may change.

Repair mathematical symbols, equations, units, signs, vector notation, MathJax/source-render mismatch, or notation consistency while preserving the intended evidence job. If correcting notation changes the Physics answer/evidence materially, fail closed and route to Core Questions instead.

Changed item -> NEEDS AUDIT when student-facing evidence changed; clear reasons; recompute summary.
