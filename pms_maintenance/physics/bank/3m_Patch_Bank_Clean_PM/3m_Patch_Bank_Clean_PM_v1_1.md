# PATCH C · CLEAN Questions — v1.1

## Purpose
Patch surface-only CLEAN items while preserving the evidence opportunity.
...
## Target selection
Only items with `audit_status == "CLEAN"` and `audit_patch_route == "patch_bank_clean"` may change.

Allowed: grammar/punctuation/article/preposition errors; remove scaffold leakage/internal labels; naturalize a context when same evidence/answer remains; clarify directions; harmless notation presentation.
Forbidden: changing evidence job, known/unknown structure, reasoning, representation, answer, choices, security relation, semantic family, DOK intent, or mapping. If required, fail closed and report that item needs a REBUILD route.

Successful changes -> NEEDS AUDIT; clear old audit reasons; recompute summary.
