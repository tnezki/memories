...
6. In the adjustable worksheet, use **All workspaces in this version** to collapse or expand every workspace in the active version, resize exact problems individually as needed, or use **↻ New Question** to replace the selected slot with another pre-generated question from that skill's curated refresh neighborhood.

## Refresh and custom behavior
The exact selected family counts control the **initial worksheet**. **↻ New Question** is a teacher edit for another question with the **same instructional intent**. It does **not** draw from every family selected elsewhere on the worksheet.

Refresh behavior is controlled by `MATH_REFRESH_GROUPS.json`. If a family belongs to a curated refresh group, its alternates may come from the explicitly authorized neighboring families in that group. If no group is defined, refresh stays in the exact family and varies legal morphology/parameters.

Example: **Factor a Quadratic** refreshes within quadratic factoring. A candidate set can intentionally include a monic factorable trinomial, a non-monic `a ≠ 1` trinomial, and a special quadratic factorization such as difference of squares or a perfect-square trinomial. It must not jump to balancing equations, exponent rules, solving a quadratic, expanding products, or graphing.

Teacher-defined custom structures use the teacher description as their own refresh neighborhood and receive pre-generated candidates. A truly new AI-authored custom question after the response ZIP has already been built would require another AI/build call, so the offline worksheet does not pretend to generate one.
