(() => {
  ...
  const TOOL_VERSION = "0.4.3-pilot";
  const REQUEST_SCHEMA = "district-math-worksheet-builder-request/0.4";
  const CONTRACT_VERSION = "district-math-worksheet-builder/1.0-pilot";
  ...
  async function buildZip(){
    ...
    try{
      const selected=selectedFamilies();
      const [contract,shared,graphStandard,catalogText,dashboardCss,worksheetCss,questionCore,generatorBank,parallelRules,refreshGroups,g12,g13,g14]=await Promise.all([
        loadText("WORKSHEET_BUILDER_CONTRACT.md"),loadText("../_shared/DISTRICT_RESPONSE_BUILD_STANDARD.md"),loadText("../_shared/DISTRICT_GRAPH_RENDERING_STANDARD.md"),loadText("question_structure_catalog.json"),loadText("dashboard_styles.css"),loadText("worksheet_styles.css"),loadGitHubRepoText("tnezki/memories","_question_structure/QUESTION_STRUCTURE_CORE.md"),loadGitHubRepoText("tnezki/memories","_question_structure/catalogs/math_worksheet_generator_bank.json"),loadGitHubRepoText("tnezki/memories","_question_structure/guides/parallel_family_rules.md"),loadGitHubRepoText("tnezki/memories","_question_structure/catalogs/math_refresh_groups.json"),loadGitHubRepoText("tnezki/memories","Tools/~graph_tool_v12.py"),loadGitHubRepoText("tnezki/memories","Tools/~graph_tool_v13.py"),loadGitHubRepoText("tnezki/memories","Tools/~graph_tool_v14.py")
      ]);
      ...
      const request={
        ...
        question_structures:{catalog_schema:catalog?.schema||"catalog-unavailable",selected_families:selected,parallel_form_policy:catalog?.parallel_form_policy||null,refresh_policy:{mode:"curated_refresh_neighborhood",candidate_count_per_slot:3,group_source:"response_contract/MATH_REFRESH_GROUPS.json",candidate_1:"initial_blueprint_family",candidate_2_3:"group_authorized_neighbors_or_same_family_fallback",forbid_worksheet_wide_selected_family_pool:true,allow_group_authorized_neighbor_even_if_not_separately_selected:true,preserve_broad_student_action:true,match_approximate_slot_difficulty:true},custom_enabled:$("customEnabled").checked,custom_course_scope:$("customEnabled").checked?browseCourses():[],custom_description:$("customEnabled").checked?$("customDescription").value.trim():null,custom_requested_count:$("customEnabled").checked?(Number($("customCount").value)||1):0},
        layout:{page_size:"US Letter portrait",two_column:$("twoColumn").checked,workspace_scale_percent:WORKSPACE_DEFAULT_PERCENT,graph_scale_percent:GRAPH_DEFAULT_PERCENT,workspace_control_range_percent:WORKSPACE_RANGE_PERCENT,version_workspace_control_range_percent:[0,300],version_workspace_default_percent:100,version_workspace_control:true,graph_control_range_percent:GRAPH_RANGE_PERCENT,independent_controls:true,full_page_workspace_capable:true},
  ...
  function buildInstructions(req){
    ...
    return `# Math Worksheet Builder Request — Pilot\n\n## Run automatically\nBuild the complete worksheet package from this ZIP. No additional teacher prompt is required. Return exactly ONE response ZIP.\n\n## Teacher request\n- Grade/course scope: ${req.worksheet.grade_course_scope.join(", ")}\n- Title: ${req.worksheet.title}\n- Learning target/focus: ${req.worksheet.learning_target||"Not specified"}\n- Questions per version: ${req.worksheet.question_count_per_version}\n- Selection model: exact selected family counts for initial generation\n- New Question refresh: curated same-intent neighborhood; exact-family fallback when no group exists\n- Difficulty profile: ${req.worksheet.difficulty_profile}\n- Versions: ${req.worksheet.version_count} (${req.worksheet.version_labels.join(", ")})\n- Answer key: ${req.worksheet.answer_key?"Yes":"No"}\n- Student layout: ${req.layout.two_column?"two columns":"one column"}\n- Initial workspace scale: ${req.layout.workspace_scale_percent}%\n- Workspace adjustment range in the finished worksheet: ${req.layout.workspace_control_range_percent[0]}%–${req.layout.workspace_control_range_percent[1]}%\n- Version-wide workspace control: ${req.layout.version_workspace_control_range_percent[0]}%–${req.layout.version_workspace_control_range_percent[1]}% (applies to all workspaces in the active version; 0% collapses all)\n- Initial graph/diagram scale: ${req.layout.graph_scale_percent}%\n\n## Exact selected question blueprint\n${fam||"No catalog families selected."}\n${req.question_structures.custom_enabled?`\n### Teacher custom structure\nScope: ${req.question_structures.custom_course_scope.join(", ")||"Use the worksheet scope"}\nCount per version: ${req.question_structures.custom_requested_count}\n${req.question_structures.custom_description}\n`:""}\n## Required authority\nFollow response_contract/QUESTION_STRUCTURE_CORE.md first for universal authoring behavior, then response_contract/MATH_WORKSHEET_GENERATOR_BANK.json, response_contract/PARALLEL_FAMILY_RULES.md, and response_contract/MATH_REFRESH_GROUPS.json for family, parallel-form, and New Question behavior, plus the worksheet and district response contracts. Use response_contract/QUESTION_STRUCTURE_CATALOG.json as the teacher-selection snapshot. All student questions, values, contexts, diagrams, and answer choices must be original.\n\nThe selected family counts are exact. Do not auto-balance, substitute, or reallocate categories. If a selected family requests 3, produce 3 legitimate instances of that family per version.\n\nFor parallel versions, the **initial displayed worksheet** must preserve the exact selected-family blueprint slot-for-slot. For each visible slot, build three independently solved candidates total. Candidate 1 stays in that slot's original blueprint family. For Candidates 2–3, resolve the seed family in **MATH_REFRESH_GROUPS.json**: use only that curated neighborhood when one exists; otherwise stay in the exact seed family and vary meaningful legal morphology/parameters. Do **not** draw from every family selected elsewhere on the worksheet. Preserve the broad student action and approximate difficulty. Teacher-defined custom structures use their own description as the refresh neighborhood.\n\nThe authoritative graph entrypoint is packaged at response_contract/graph_tool/~graph_tool_v14.py with v13 and v12 beside it. Use it for supported Cartesian graph needs and preserve the locked Cartesian print weights in request.json.\n\nCopy both locked CSS files byte-for-byte to the response assets and verify their hashes in QA. The student worksheet HTML must include an **All workspaces in this version** control from 0% through 300%, plus the existing per-problem workspace and graph/diagram controls. The version-wide control applies only to the active version, preserves graph/diagram scales, immediately repaginates, and lets 0% collapse every workspace in that version at once. Per-problem workspace remains adjustable from 0% through 1200% for individual overrides/full-page expansion.\n\n## Required files\n${req.resolved_outputs.required_files.map(p=>`- ${p}`).join("\n")}\n\nUse conventional geometry marks in every preview/worksheet/answer figure: perpendicular tick strokes for congruent segments, arrowhead/chevron marks for parallel segments, and square right-angle markers. Do not use star/asterisk/X-like glyphs as geometry marks.\n\nVisually inspect the adjustable worksheet and answer key in screen and print modes. Return only the completed response ZIP.`;
  }
