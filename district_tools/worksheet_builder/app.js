(() => {
  ...
  const TOOL_VERSION = "0.4.2-pilot";
  const REQUEST_SCHEMA = "district-math-worksheet-builder-request/0.4";
  const CONTRACT_VERSION = "district-math-worksheet-builder/0.9-pilot";
  ...
  async function buildZip(){
    ...
    try{
      ...
      const request={
        ...
        question_structures:{catalog_schema:catalog?.schema||"catalog-unavailable",selected_families:selected,parallel_form_policy:catalog?.parallel_form_policy||null,refresh_policy:{mode:"selected_family_pool",candidate_count_per_slot:3,candidate_1:"initial_blueprint_family",candidate_2_3:"any_teacher_selected_family",allow_post_refresh_family_count_drift:true,forbid_unselected_families:true,preserve_selected_course_scope:true,match_approximate_slot_difficulty:true},custom_enabled:$("customEnabled").checked,custom_course_scope:$("customEnabled").checked?browseCourses():[],custom_description:$("customEnabled").checked?$("customDescription").value.trim():null,custom_requested_count:$("customEnabled").checked?(Number($("customCount").value)||1):0},
        ...
        authority_note:"The exact selected-family counts are the initial worksheet blueprint. The New Question control may later switch a slot among teacher-selected families according to refresh_policy; unselected families are forbidden. Question Structure Core and the Math Worksheet Generator Bank control family behavior. All authored problems remain original."
      };
      const entries=[
