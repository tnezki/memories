...
def audit_main(p:Path,errors:list[str]):
    raw=p.read_text(encoding='utf-8',errors='replace'); s=BeautifulSoup(raw,'html.parser')
    links=[x.get('href','') for x in s.find_all('link',rel=lambda v:v and 'stylesheet' in v)]
    if links!=['../../css/review.css']: err(errors,f'{p}: stylesheet contract must be exactly ../../css/review.css; got {links}')
    if not s.select_one('header.hero .hero-inner .jump-nav'): err(errors,f'{p}: missing canonical hero/jump-nav shell')
    for rid in ('glance','vocabulary','structures','dok','final-check'):
        if not s.select_one(f'section#{rid}.review-panel'): err(errors,f'{p}: missing #{rid}.review-panel')
    ...
    if any(re.fullmatch(r'MG\s*\d+',x,re.I) for x in nav): err(errors,f'{p}: raw MG-number navigation is prohibited')
    if not s.select_one('a.floating-top[href="#top"]'): err(errors,f'{p}: missing floating Top control')
...
def audit_practice(p:Path,errors:list[str]):
    s=BeautifulSoup(p.read_text(encoding='utf-8',errors='replace'),'html.parser'); links=[x.get('href','') for x in s.find_all('link',rel=lambda v:v and 'stylesheet' in v)]
    if links!=['../../css/review_practice.css']: err(errors,f'{p}: stylesheet contract must be exactly ../../css/review_practice.css; got {links}')
    if not s.select('section.dok-section[data-mastery-goal]'): err(errors,f'{p}: practice sections missing data-mastery-goal')
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('root'); args=ap.parse_args(); root=Path(args.root); errors=[]; mains=[]; practices=[]
