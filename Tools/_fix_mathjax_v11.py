# ~fix_mathjax_v11.py - v10 base + safe multi-row/system MathJax repair
...
#   python ~fix_mathjax_v10.py /absolute/path/to/folder
...
# V11 - SAFE MULTI-ROW / SYSTEM REPAIR
#
# V10 could mistake a legitimate row break immediately followed by a variable,
# such as \\x + 2y = 2 inside a cases/aligned environment, for a doubled TeX
# command slash and collapse it to the invalid command \x. V11 only collapses
# doubled slashes before known TeX command names and repairs high-confidence
# already-corrupted one-letter row starts inside explicit multi-row environments.
...
CORRUPTED_SYSTEM_ROW_RE = re.compile(
    r'(?<!\\)\\(?P<var>[A-Za-z])'
    r'(?=\s*(?:[-+*/=]|\\(?:le|ge|lt|gt|ne|neq)\b))'
)


def repair_corrupted_system_rows(content):
    """Repair high-confidence \\x / \\y / \\z style row-break corruption."""
    if not MULTIROW_ENV_RE.search(content):
        return content, 0
    ...
    def repl(match):
        nonlocal hits
        hits += 1
        return r'\\ ' + match.group('var')

    return CORRUPTED_SYSTEM_ROW_RE.sub(repl, content), hits
...
def repair_math_blocks(text):
    ...
    def repl(match):
        ...
        # V11) Repair already-corrupted system row breaks such as
        #      \x + 2y = 2  ->  \\ x + 2y = 2
        content, n = repair_corrupted_system_rows(content)
        if n:
            counts['system_row'] += n
        ...
        # 4) Collapse two literal backslashes ONLY before a known TeX command.
        #    Do NOT collapse row breaks such as \\x in cases/aligned systems.
    ...
    hits = []
    if counts['system_row']:
        hits.append(
            f"         {counts['system_row']}x corrupted multi-row/system break repaired "
            "(for example \\x -> \\\\ x inside cases/aligned math)"
        )
    if counts['double_tex']:
        hits.append(
            f"         {counts['double_tex']}x doubled slash collapsed before known TeX command"
        )
    if counts['tabs']:
...
def audit_file(path, text):
    ...
    # Doubled slashes before known TeX commands should be gone after repair.
    # Legitimate multi-row breaks such as \\x are not doubled-command errors.
    doubled = sum(len(DOUBLED_KNOWN_TEX_RE.findall(b)) for b in math_blocks)
    if doubled:
        warnings.append(
            f"    DOUBLED TEX ESCAPE: {doubled} known-command instance(s) remain inside math"
        )

    corrupted_rows = 0
    for b in math_blocks:
        if MULTIROW_ENV_RE.search(b):
            corrupted_rows += len(CORRUPTED_SYSTEM_ROW_RE.findall(b))
    if corrupted_rows:
        warnings.append(
            f"    CORRUPTED SYSTEM ROW BREAK: {corrupted_rows} instance(s) like \\x remain "
            "inside cases/aligned/array math"
        )
    ...
    # V10: content normalized to $ / $$ requires matching MathJax config.
...
SUPPORTED_SUFFIXES = {'.html', '.txt', '.csv', '.json'}
text_files = [
...
if not text_files:
    ...
    print(
        ...
        f"(.html/.txt/.csv/.json) in: {folder.resolve()}\n"
    )
...
for path in text_files:
    ...
    # V9 JSON-safe canonical-data repair. Do NOT run generic MathJax slash/delimiter
    # normalization on JSON source because TeX backslashes are JSON-escaped. Only
    # repair the known Unicode command-tail corruption with correctly escaped TeX.
    if path.suffix.lower() == '.json':
        json_before = text
        text = re.sub(r'(?:≤|&le;|&#8804;)\s*ft(?=\s*[({\[])', r'\\\\left', text)
        text = re.sub(r'(?:≥|&ge;|&#8805;)\s*ight(?=\s*[)}\]])', r'\\\\right', text)
        if text != json_before:
            hits.append('         JSON-safe corrupted left/right TeX command tail repaired')
        ...
        try:
            ...
            print(f"  AUDIT  {path}\n    INVALID JSON AFTER REPAIR: {exc}")
            audit_count += 1
        continue

    # V10 — keep the HTML MathJax configuration aligned with this fixer's
    # canonical $ / $$ content normalization.
    if path.suffix.lower() == '.html':
        text, config_hits = repair_mathjax_config_html(text)
        if config_hits:
            hits.append(
                f"         {config_hits}× MathJax config normalized to $ / $$ delimiters"
            )

    # Section 3a — V8: escape high-confidence prose currency BEFORE any
