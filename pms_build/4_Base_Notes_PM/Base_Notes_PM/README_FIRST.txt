...
-----------------------
Run the current Framework `tools/~fix_mathjax_v10.py` on the complete staging Notes output before creating the final ZIP; then QA the repaired bytes.

Existing Notes repair helper
----------------------------
Use `~fix_notes_layout_contract_v2.py` to repair legacy vocabulary-sort and skim-table markup without changing CSS.
