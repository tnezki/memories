...
--------------------
Before packaging, run current Framework `tools/MathJax/final-byte fixer declared by the current Framework manifest` on both staging resource roots (or their common staging parent), then rerun QA and ZIP only the repaired bytes.
