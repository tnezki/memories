...
for p in root.rglob('*.html'):
    ...
    # Exact universal Activity hub shell: proven Algebra implementation.
    if name.endswith('_act1.html') and 'questions_solutions' not in name:
        wrap=s.body.find('div',class_='wrap',recursive=False) if s.body else None
        check(bool(wrap),f'{p}: Activity hub body missing direct .wrap')
        if wrap:
            pages=wrap.find_all('section',class_='page',recursive=False)
            expected=['activity-options','teacher-student-moves','dir-whiteboard-indy','dir-whiteboard-partners',
                      'dir-rally-coach','dir-speed-dating-math','dir-showdown','dir-think-trade-agree','dir-round-table',
                      'dir-mathematical-hot-seat','dir-rally-coach-ii','dir-desmos-activity','dir-stations',
                      'dir-find-someone-who','dir-tarsia-puzzle-activity']
            ids=[x.get('id','') for x in pages]
            check(ids==expected,f'{p}: Activity hub page IDs/order must match canonical 15-page shell; got {ids}')
        for cls in ('dir-header','act-eyebrow','act-title','act-subtitle','act-meta','dir-text','dir-row','dir-label'):
            check(bool(s.select_one('.'+cls)),f'{p}: Activity hub missing canonical .{cls} hook')

    # Exact Questions/Solutions shell: one question page then matching solution page for every routed item.
    if 'questions_solutions' in name:
        wrap=s.body.find('div',class_='wrap',recursive=False) if s.body else None
        check(bool(wrap),f'{p}: Questions/Solutions body missing direct .wrap')
        if wrap:
            pages=wrap.find_all('section',class_='page',recursive=False)
            qpages=[x for x in pages if 'question-page' in (x.get('class') or [])]
            spages=[x for x in pages if 'solution-page' in (x.get('class') or [])]
            check(len(qpages)==len(spages) and len(qpages)>0,f'{p}: Questions/Solutions requires equal nonzero question/solution pages')
            if len(qpages)==24 or len(spages)==24:
                check(len(pages)==48 and len(qpages)==24 and len(spages)==24,
                      f'{p}: 24-item route must render 48 alternating pages (24 question + 24 solution)')
            for i,page in enumerate(pages):
                expect='question-page' if i%2==0 else 'solution-page'
                check(expect in (page.get('class') or []),f'{p}: page {i+1} must be {expect} in strict Q/S alternation')
            for i,q in enumerate(qpages,1):
                for cls in ('prob-head','prob-num','prob-q','work'):
                    check(bool(q.select_one('.'+cls)),f'{p}: question page {i} missing .{cls}')
            for i,a in enumerate(spages,1):
                for cls in ('prob-head','prob-num','prob-q','solution'):
                    check(bool(a.select_one('.'+cls)),f'{p}: solution page {i} missing .{cls}')
    ...
    if 'find_someone_who' in name:
        ...
        for i,page in enumerate(spages,1):
            cls=set(page.get('class') or [])
            check({'page','worksheet','student-worksheet-page'}.issubset(cls),f'{p}: student worksheet page {i} missing canonical page/worksheet classes')
            for hook in ('worksheet-title','section-label','page-footer'):
                check(bool(page.select_one('.'+hook)),f'{p}: student worksheet page {i} missing .{hook}')
            if i==1:
                check(bool(page.select_one('.intro')),f'{p}: first student worksheet page missing canonical .intro directions/name block')
        for i,page in enumerate(kpages,1):
            check('page' in (page.get('class') or []),f'{p}: teacher key page {i} missing canonical .page wrapper')
    ...
    if 'stations' in name:
        ...
        if wrap:
            ...
            for j in range(0,len(prompt_region),2):
                ...
                for hook in ('copy-note','header','station-grid','page-footer'):
                    check(bool(qpage.select_one(':scope > .'+hook)),f'{p}: station prompt page {j//2+1} missing canonical .{hook}')
