...
Behavior:
- Finds Summative selected-response items using the current canonical Bank signals
  (`destination`/Summative security + `question_design.response_mode`) with
  legacy `item_type` support.
...
def is_summative_item(item: dict) -> bool:
    dest_group = normalize_token(item.get("destination_group"))
    dest = normalize_token(item.get("destination"))
    security = normalize_token(item.get("security_level"))
    return (
        dest_group == "summative"
        or dest.startswith("summative")
        or "summative" in security
        or bool(item.get("summative_version"))
        or bool(item.get("destination_form_set_version"))
    )


def selected_response_signal(item: dict) -> bool:
    qd = item.get("question_design") if isinstance(item.get("question_design"), dict) else {}
    response_mode = normalize_token(qd.get("response_mode") or item.get("response_mode"))
    item_type = normalize_token(item.get("item_type"))
...
def is_summative_sr(item: dict) -> bool:
    ...
    if not isinstance(choices, list) or len(choices) < 2:
        return False
    return is_summative_item(item) and selected_response_signal(item)
...
def run(data, seed: str, strict_16x4: bool = False):
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        choices = item.get("choices")
        if is_summative_item(item) and isinstance(choices, list) and len(choices) >= 2:
            summative_choice_candidates.append(item)
        if is_summative_sr(item):
            groups[form_id(item)].append(item)
            recognized_ids.add(id(item))

    if strict_16x4:
        missed = [it for it in summative_choice_candidates if id(it) not in recognized_ids]
        if missed:
            sample = ", ".join(str(it.get("bank_id", "<unknown>")) for it in missed[:8])
            raise ValueError(
                "strict-16x4 found Summative records with choices that were not recognized as "
                f"selected response ({len(missed)} missed; sample: {sample}). "
                "Check question_design.response_mode/item_type schema signals."
            )
        if summative_choice_candidates and not groups:
            raise ValueError(
                "strict-16x4 found Summative records with choices but recognized zero selected-response items; "
                "PASS_NOOP is forbidden for this condition."
            )
    ...
    if not groups:
        return {
            "status": "PASS_NOOP",
            "tool_version": TOOL_VERSION,
            "detector": "summative_destination+question_design.response_mode_or_item_type+choices",
            "forms": {},
            "items_changed": 0,
        }
