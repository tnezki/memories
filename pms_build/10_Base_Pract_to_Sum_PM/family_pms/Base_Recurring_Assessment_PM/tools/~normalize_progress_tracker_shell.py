...
def clean_leading_check(text: str) -> str:
    return re.sub(rf"^[\s{re.escape(CHECK_GLYPHS)}]+","",text or "").strip()
...
def normalize(path: Path):
    ...
    for row in soup.select(".ican-list .ican"):
        ...
        # Rebuild exact two-cell DOM expected by progress_tracker.css.
        row.clear()
        check=soup.new_tag("span",attrs={"class":"check","aria-hidden":"true"})
        txt=soup.new_tag("span",attrs={"class":"ican-text"})
        txt.string=text
        row.append(check)
        row.append(txt)

    # CSS .status-choice::before supplies the visual checkbox.
...
def audit(path: Path):
    ...
    for i,row in enumerate(soup.select(".ican-list .ican"),1):
        ...
        if len(children)!=2 or not check or not text:
            errors.append(f"I-can row {i} must have exactly .check + .ican-text")
            continue
        if check.get_text(strip=True):
            errors.append(f"I-can row {i} .check must be empty")
        if not re.match(r"^I\s+can\b",text.get_text(" ",strip=True),re.I):
            errors.append(f"I-can row {i} text must begin with 'I can'")

    choices=soup.select(".status-row .status-choice")
    if len(choices)!=4:
        errors.append(f"Expected 4 status choices, found {len(choices)}")
    for c in choices:
        if re.search(r"[□☐☑✓✔]",c.get_text()):
            errors.append("Status choice contains checkbox glyph even though CSS supplies it")
