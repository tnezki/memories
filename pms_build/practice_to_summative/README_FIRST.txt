...
Current refinement: v3.6 keeps the quality/anti-confusion gates and makes the master ZIP a Finder-friendly drop-in package using the deployed course-parent folders.
...
V3.5: The PM remains universal across Algebra, Calculus, and Physics. The Framework selects the Summative layout/profile. Shared course-root CSS remains external to Unit ZIPs. Selected-response Summative choices are deterministically rebalanced with the bundled `Tools/scramble_selected_response.py` on a staging copy immediately before final Summative rendering/QA/ZIP.

V3.5 DETECTOR FIX: the bundled selected-response scrambler reads the current canonical Bank schema (`question_design.response_mode`) and fails closed in strict Physics mode if Summative choice records are present but not recognized.

V3.6: Locks the approved universal shells instead of repairing Physics with CSS. Physics-specific profile now requests ws-50 only for Practice 1/2/3/Extra, required Tarsia/Blooket resolution, universal Activity/Exit mechanics, a two-page physical Progress Tracker, and materialized Summative bubble rows. Review/Warm-Ups remain unchanged.
