    ...
    for p in candidates:
        ...
        if isinstance(value, str):
            ...
            if HEX64.fullmatch(value):
                valid.append((p, value))
    hashes = sorted({v for _, v in valid})
    if len(hashes) == 1:
        return {
            "method": "stage3e-canonical-v1",
            "fingerprint": f"stage3e-canonical:{hashes[0]}",
            "source": str(valid[0][0].relative_to(root).as_posix()),
        }
    if len(hashes) > 1:
        raise SystemExit("Conflicting canonical_authored_content_sha256 values found in STAGE_3E_HANDOFF.json files")
    return None
...
def manifest_identity(root: Path):
    ...
    for rel, digest in rows:
        ...
        h.update(b"\n")
    return {
        "method": "content-manifest-v1",
        "fingerprint": f"content-manifest:{h.hexdigest()}",
        "file_count": len(rows),
    }
...
def main():
    ...
    ap.add_argument("bank_root", help="Extracted/materialized unitN Bank folder")
    ap.add_argument("--require-stage3e", action="store_true", help="Fail instead of falling back when no valid Stage 3E canonical hash exists")
    args = ap.parse_args()
    root = Path(args.bank_root).resolve()
    if not root.is_dir():
        raise SystemExit(f"Bank root is not a directory: {root}")
    result = stage3e_identity(root)
    if result is None:
        if args.require_stage3e:
            raise SystemExit("No valid Stage 3E canonical_authored_content_sha256 found")
