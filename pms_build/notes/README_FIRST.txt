...
- Physics course resource: `resources/Physics_Notes_Image_Toolkit/` (resolved by the Physics Framework manifest)

The 4A map owns story/image/processing-anchor placement. The Unit Bank owns WTC and EX/YTI content. The course-owned image toolkit owns source-image inventory/provenance and stays in `physics/resources/`; 4B references those assets in place rather than duplicating them in Notes. The Framework owns Physics Notes behavior, resource paths, and deployed layout contracts.
...
v1.4 COURSE-OWNED IMAGE REFERENCES
- Toolkit root is resolved from the Physics Framework manifest in the pinned Physics repo.
- Selected toolkit images are referenced at repository-relative paths; they are not copied into Notes section folders.
- A build runtime does not need a binary connector-to-sandbox handoff for toolkit images.
