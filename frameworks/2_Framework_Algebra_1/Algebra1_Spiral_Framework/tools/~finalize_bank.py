...
def checks(base,residual_math):
    ...
    req=[base/f'unit{unit}.html',base/f'unit{unit}_review.html',base/'generate_graphs.py',base/'~finalize_bank.py']+[base/f'unit{unit}{s}' for s in REQUIRED_SUFFIXES]
    ...
    counts['mapping_rows']=len(mapping)
    htmlp=base/f'unit{unit}.html';reviewp=base/f'unit{unit}_review.html'
    ht=htmlp.read_text(encoding='utf-8',errors='replace');rht=reviewp.read_text(encoding='utf-8',errors='replace')
    article_re=r'<article\b[^>]*class=["\'][^"\']*\bbank-item\b[^>]*>'
    articles=len(re.findall(article_re,ht,re.I));review_articles=len(re.findall(article_re,rht,re.I))
    counts['html_bank_items']=articles;counts['review_html_bank_items']=review_articles
    if articles==review_articles==len(mapping)==len(items):ck['html_mapping_json']=f'PASS ({articles} in both HTML views)'
    else:
        ck['html_mapping_json']='FAIL'
        hard.append({'where':'PACKAGE','issue':f'Canonical counts disagree: teaching HTML={articles}, review HTML={review_articles}, mapping={len(mapping)}, JSON={len(items)}'})
    mathok=True;balanced=True
    for label,doc in [('TEACHING HTML',ht),('REVIEW HTML',rht)]:
        this_math="inlineMath: [['$','$']]" in doc and "displayMath: [['$$','$$']]" in doc
        body=SCRIPT_STYLE_RE.sub('',doc);this_balanced=len(re.findall(r'(?<!\\)\$',body))%2==0
        if not this_math:hard.append({'where':label,'issue':'MathJax head is not configured for $...$ / $$...$$.'})
        if not this_balanced:hard.append({'where':label,'issue':'Unbalanced unescaped $ delimiters remain in HTML.'})
        mathok=mathok and this_math;balanced=balanced and this_balanced
    if residual_math:
    ...
    ids=set();practice_tails=defaultdict(list);practice_iteration=[]
    for item in items:
...
def write_report(base,status,counts,hard,review,graph_ok,cleanup_ok,final_zip=None):
    ...
    for label,key in [('Required files','required_files'),('Teaching/review HTML / mapping / JSON','html_mapping_json'),('MathJax / HTML','mathjax_html'),('Resolved Algebra inventory','locked_inventory'),('Exports','exports'),('Summative profile','summative_profile'),('Question review scan','question_review')]:lines.append(f'- {label}: {ck.get(key,"?")}')
...
def main():
    ap=argparse.ArgumentParser(description='Algebra Bank finalizer. Run with no argument from inside unitN/, or optionally pass the extracted unitN/ folder.')
