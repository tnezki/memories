...
Student-facing wording must instantiate the family `generator.prompt_pattern` with concrete mathematical values, expressions, data, context, and/or figures.
...
`catalogs/math_family_preview_specimens.json` stores solved representative morphology specimens keyed by canonical `family_id`.
...
Teacher-facing preview UIs MUST use a curated specimen when one exists. If one does not exist, show **Preview pending quality review** rather than inventing a fallback question.
